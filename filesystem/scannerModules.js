...
const { formatRJCode, getWorkDuration } = require("./utils");
...
async function getMetadata(folder, tagLanguage) {
  ...
  logger.task.info(rjcode, "从 DLSite 获取元数据...");

  let metadata = null;

  try {
    metadata = await scrapeWorkMetadataFromDLsite(folder.id, tagLanguage); // 抓取该音声的元数据
    metadata.duration = await getWorkDuration(folder.id, folder.absolutePath); // 获取本地文件夹音色播放总时长
  } catch (err) {
    logger.task.warn(rjcode, `DLSite获取元数据失败: ${err.message}`);
  }
  if (metadata == null) {
    try {
      metadata = await scrapeWorkMetadataFromAsmrOne(folder.id, tagLanguage);
    } catch(err) {
      logger.task.warn(rjcode, `AsmrOne获取元数据失败: ${err.message}`)
    }
  }

  logger.task.info(rjcode, "元数据抓取成功，准备添加到数据库...");
  ...
  try {
    ...
    logger.task.error(rjcode, `元数据添加失败: ${err.message}`);
    return "failed";
  }
...
async function processFolder(folder) {
  const res = await db.knex("t_work").select("id").where("id", "=", folder.id).count().first();
  ...
  try {
    ...
    await Promise.all(uniqueFolderList.map(async (folder) => {
      ...
      const rjcode = folder.id; // zero-pad to 6 digits\
        switch(result) {
          case 'added': logger.task.info(rjcode, `添加成功! Added: ${counts.added}`); break;
          case 'failed': logger.task.error(rjcode, `添加失败! Failed: ${counts.failed}`); break;
          default: break;
        }
